...
The clock also incorporates the ability to set a UTC offset (defaults to -360 minutes) so it can be used in any geography.  Additional preference settings include brightness, 12- or 24-hour display mode, and if the display orientation is flipped.

## Setup
The clock only requires that the antenna be oriented to its long axis is perpendicular to the direction facing Wellington, CO, USA (40°41′54″N 105°00′01″W]).  After power-on, it may take up to 3 minutes to synchronize the RTC.  If the signal is poor of or the antenna is mis-aimed, it may take longer or synchronization may not be achieved.

If the clock is not synchronized, the decimal point between the tens and ones seconds will be illuminated.  Similarly, at power-up, the displayed time will be 00.00.00 and the seconds will increment as new received pulses are received.

## Configuration
Pressing the configuration button for more than one second, will switch the clock to configuration mode.  Press the button for more than one second to move to the next option.  Short-ckick the button to change the option's value.
...
## Troubleshooting
* If the clock is not updating or the seconds are increasing rapidly, it is an indication there is too much interference, or the antenna is not aimed properly.
* If the clock is not updating and reboots after 60 seconds, no radio signal is being received.
...
2. Use hot-glue to affix the display and ESP32 to the case.  
3. There is a single layer of print in front of the display that acts as a diffuser.  Press down on the display while the glue cools to make sure it is clearly visible from the outside of the case.
4. Tighten the cam nut to the screw first and then lock it into place with
the smaller nut.  Tighten so the antenna holder requires some light force to rotate it, but do not overtighten - printed screws aren't very sturdy.
5. Use M2.5 heated inserts in the rear of the case.
6. Make sure there aren't any obstructions that prevent the antenna from turning when the case is assembled - for example the display wires.  Also use enough connecting wire between the antenna module to make a loop in the case before connecting to the ESP32.  This affords the ability to rotate the antenna.
